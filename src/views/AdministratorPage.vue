    ...
    <div class="swiper third-swipe">
      <div class="swiper-wrapper">
        <div class="swiper-slide swiper-third">
          ...
          <div class="car-tuning2" v-for="index of 3" :key="index">
...
export default {
  ...
  methods:{
    ...
    jumpGameBase(user_id){
      ...
      this.$router.push({name:'AdminBasePage', query: {userID:user_id}});
    },
            ...
            switch (res.data.result){
              ...
              case 1:
               alert("获取用户" + user_id + "id" + "成功");
                this.user_id.push(res.data.user_id);
                break;
            }
          })
    },
     ...
     alert('gid 不能为空')
      ...
      this.$axios.post('api/gamedetail/getGameComments', {
        ...
        for (let i in res.data.comment_list) {
          if (res.data.comment_list[i].name === 'BASIC_ORDER' || res.data.comment_list[i].name === 'BASIC_COMMENT' || res.data.comment_list[i].name === 'BASIC_USER') {
            this.comment_num -= 1;
            continue;
          }
    ...
    async searchUser(){
      ...
      await this.getUserInfo(this.user_id[0]);

      this.user_cover.push('User/' + this.user_id[0] + '/ProfilePhoto.jpg');
    },
